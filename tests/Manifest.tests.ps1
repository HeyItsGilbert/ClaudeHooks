BeforeAll {
    ...
    # Pre-Specify RegEx Matching Patterns
    $gitTagMatchRegEx   = 'tag:\s?.(\d+(\.\d+)*)' # NOTE - was 'tag:\s*(\d+(?:\.\d+)*)' previously
    $changelogTagMatchRegEx = "^##\s\[(?<Version>(\d+\.){1,3}\d+)\]"

    $moduleName         = 'ClaudeHooks'
    $manifestPath       = Join-Path $PSScriptRoot '..\ClaudeHooks\ClaudeHooks.psd1'
    ...
    $changelogPath    = Join-Path -Path $PSScriptRoot '\..\CHANGELOG.md'
